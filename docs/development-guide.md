...
The Pyterraformer project uses Docker to ease setting up a consistent development environment. The Docker documentation has
...
In the devbox environment you'll be able to enter a python shell and import `pyterraformer` or any dependencies.
...
`pyterraformer` is [PEP 517][pep-517] compliant. [build][build] is used as the frontend tool for building the library.
...
* **requirements-docs.txt** - Lists all dependencies needed to build the documentation.
...
# Step 2: Rebuild containers with new dependencies
docker-compose build devbox
```

When updating `requirements-test.txt` or `requirements-docs.txt`:

```bash
# Only rebuild is needed (no lock file update)
docker-compose build devbox
```
...
2. Have an environment configured for Python 3.10 or later.
3. Perform the version bump part name (`major`, `minor`, or `patch`).
    Example: `hyper-bump-it by minor`

    This will create a new branch, updates all affected files with the new version, commit the changes to the branch, and 
    push the branch.
...
includes: [MyPy][mypy-docs] and [Ruff][ruff-docs].
...
[code of conduct]: https://github.com/wayfair-incubator/pyterraformer/blob/main/CODE_OF_CONDUCT.md
[contributing]: https://github.com/wayfair-incubator/pyterraformer/blob/main/CONTRIBUTING.md
...
[pypi]: https://pypi.org/project/pyterraformer/
